...
from sample.constants import MODEL_ATTRS, TITLES
...
class ExcelHandler:
    ...
    def _deal_with_headers(self, cells):
        ...
        return [
            MODEL_ATTRS[TITLES.index(_compare_name(cell.value))]['key']
...
class ValueProcess:
    ...
    def _handle_functions(self):
        return {
            ...
            'sequence_size': lambda x: round(x, 2),
            'real_size': lambda x: round(x, 2),
            'quality_30': lambda x: round(x, 2),
            'user_id': self._handle_user,
        }
...
def export_to_csv(querset):
    headers = [a['name'] for a in MODEL_ATTRS]

    data = [headers]

    for o in querset:
        data.append(
            [getattr(o, a.get('alias', a['key'])) for a in MODEL_ATTRS])
