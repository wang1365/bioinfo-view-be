...
class ReportView(CustomeViewSets):
    ...
    def create(self, request, *args, **kwargs):
        ...
        def async_create_report(report):
            ...
            json_filepath = os.path.join(
                report.task.result_dir, "report", str(report.id)
            )
            os.makedirs(os.path.dirname(json_filepath), exist_ok=True)
            with open(json_filepath, "w") as fp:
                fp.write(data["query"])

            report_file_path_prefix = os.path.join(
                report.task.result_dir, "report", f"{report.id}"
            )
        ...
        threading.Thread(
            target=async_create_report, args=(report,), daemon=True
        ).start()
    ...
    def list(self, request, *args, **kwargs):
        ...
        sample_meta_identifier = request.GET.get("sample_meta_identifier", None)
        ...
        if sample_meta_identifier:
            filtered = True
            samples = samples.filter(sample_meta__identifier=sample_meta_identifier)
...
def get_report_by_sample(request):
    """
    GET /report/report/by_sample?task_id=...&sample_id=...
    Returns existing report for task+sample combination or null
    """
    task_id = request.GET.get("task_id")
    sample_id = request.GET.get("sample_id")

    if not task_id:
        return response_body(msg="task_id is required", code=-1)

    try:
        if sample_id:
            report = Report.objects.get(task_id=task_id, sample_id=sample_id)
        else:
            report = Report.objects.get(task_id=task_id, sample__isnull=True)
        serializer = ReportSerializer(report)
        return response_body(data=serializer.data, msg="success")
    except Report.DoesNotExist:
        return response_body(data=None, msg="Report not found")


def upsert_report_by_sample(request):
    """
    POST /report/report/by_sample
    Upserts report for task+sample and triggers report generation
    """
    ...
    try:
        ...
        return response_body(msg="Task not found", code=-1)

    # Check if sample exists when sample_id is provided
    sample = None
    if sample_id:
        try:
            sample = SampleData.objects.get(id=sample_id)
        except SampleData.DoesNotExist:
            return response_body(msg="Sample not found", code=-1)

    script_path = "/data/bioinfo/database_dir/individual_report.py"
    if not os.path.isfile(script_path):
        return response_body(msg="报告脚本不存在", code=-1)
    ...
    if sample_id:
        report, created = Report.objects.update_or_create(
            task=task, sample=sample, defaults=report_data
        )
    else:
        report, created = Report.objects.update_or_create(
            task=task, sample__isnull=True, defaults=report_data
        )

    def async_create_report(report, query_data):
        # save query to a txt file
        json_filepath = os.path.join(report.task.result_dir, "report", str(report.id))
        os.makedirs(os.path.dirname(json_filepath), exist_ok=True)
        with open(json_filepath, "w") as fp:
            fp.write(query_data)
    ...
    threading.Thread(
        target=async_create_report, args=(report, query), daemon=True
    ).start()
...
def report_by_sample(request):
    """Unified endpoint for by-sample reports.
    - GET: return existing report for task_id and sample_id (query params)
    - POST: upsert report for task_id and sample_id (body payload)
    """
    if request.method == "GET":
        return get_report_by_sample(request)
    elif request.method == "POST":
        return upsert_report_by_sample(request)
    else:
        return response_body(msg="Method not allowed", code=-1, status=405)
